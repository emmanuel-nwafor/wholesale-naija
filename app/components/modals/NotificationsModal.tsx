...
import { CheckCircle, Star, Megaphone, X, Package, Bell } from 'lucide-react';
...
export default function NotificationsModal({
  ...
  useEffect(() => {
    ...
    const fetchNotifications = async () => {
      try {
        setLoading(true);
        const res = await fetchWithToken<{ notifications: Notification[] }>(
          '/v1/notifications'
        );
    ...
    switch (type) {
      case 'productApproved':
        return {
          ...
          title: (n: Notification) =>
            `Product "${n.payload.productName || 'Unknown'}" approved!`,
      ...
      case 'rating':
        return {
          ...
          title: (n: Notification) =>
            `New ${n.payload.rating || 5} star review`,
                      ...
                      >
                        ...
                        <div className="flex gap-3">
                          ...
                          <div className="flex-1">
                            <p
                              className={`text-sm font-medium ${!notif.read ? 'font-semibold' : 'text-gray-900'}`}
                            >
                              {config.title(notif)}
                            </p>
