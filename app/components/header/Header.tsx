...
export default function Header() {
  ...
  useEffect(() => {
    ...
    setToken(t);
    if (t) loadUserProfile(); // FIX 1: loadUserProfile is now included in dependencies below
  ...
  useEffect(() => {
    ...
    const fetchSearch = async () => {
      ...
      } catch (err: unknown) { // FIX 2: Used 'unknown' instead of 'any'
    ...
    <>
      ...
      <div className="bg-[url('https://res.cloudinary.com/dqtjja88b/image/upload/v1760883994/3680b6919f2237dd1d7bcfe119a8522af6738e96_2_gtqmc2.jpg')] bg-cover bg-center h-20 sm:h-20 md:h-24 relative overflow-hidden">
        <div className="relative z-10 flex items-center justify-between px-2 sm:px-4 md:px-10 h-full">
          <Link href="/" className="flex items-center gap-3">
            <Image src="/svgs/logo.svg" alt="Wholesale Naija" width={170} height={48} className="h-12 sm:h-14 md:h-16 w-auto" />
          </Link>
          ...
          <button onClick={() => setSidebarOpen(true)} className="md:hidden p-1 sm:p-2 text-white">
      ...
      <div className="bg-slate-800 px-2 sm:px-4 md:px-10 py-3 sm:py-4 flex flex-col md:flex-row items-center justify-between gap-3">
        ...
        <div className="flex gap-3 w-full md:w-auto items-center">
          {user ? (
            <>
              <button onClick={() => router.push('/product/saved')} className="p-2 text-white hover:text-gray-300">
                <Image src="/svgs/heart.svg" alt="heart" width={40} height={40} />
              </button>

              <button onClick={() => router.push('/messages')} className="p-2 text-white hover:text-gray-300">
                <Image src="/svgs/message.svg" alt="message" width={40} height={40} />
              </button>

              <button onClick={navigateToWallet} className="flex items-center gap-2 bg-gray-700 text-white px-2 py-2 rounded-2xl font-semibold">
                <Image src="/svgs/coin-1.svg" alt="coin" width={25} height={25} />
                {balanceLoading ? <p className="text-xs animate-pulse">...</p> : <p className="text-xs">{walletBalance} Coins</p>}
              </button>

              <button
                onClick={() => router.push(user?.role === 'seller' ? '/store/dashboard' : '/profile')}
              ...
              >
                <div className="w-8 h-8 sm:w-9 sm:h-9 relative flex-shrink-0">
                  <Image src={user.profilePicture?.url || '/svgs/logo.svg'} alt="User" fill className="rounded-full object-cover" />
                </div>
            ...
            <>
              <button onClick={() => setLoginModalOpen(true)} className="flex-1 md:flex-initial border border-white text-white px-6 py-3 rounded-xl hover:bg-white hover:text-slate-800 transition">Login</button>
              <button onClick={openEmailModal} className="flex-1 md:flex-initial bg-white text-slate-800 px-6 py-3 rounded-xl font-semibold hover:bg-gray-100 transition">Create Account</button>
            </>
          )}
        </div>
      </div>
      ...
      <AnimatePresence>
        {loginModalOpen && <ChooseModal key="login-modal" isOpen={loginModalOpen} onClose={() => setLoginModalOpen(false)} onSelectBuyer={() => {}} onSelectSeller={() => {}} onOpenLogin={() => { setLoginEmailOpen(true); setLoginModalOpen(false); }} />}
        {signupChooseModalOpen && <ChooseModal 
            key="signup-choose-modal" 
            isOpen={signupChooseModalOpen} 
            onClose={() => setSignupChooseModalOpen(false)} 
            onSelectBuyer={() => { // FIX 3: Wrapped expressions in {}
            ...
            onSelectSeller={() => { // FIX 3: Wrapped expressions in {}
            ...
            onOpenLogin={() => {}} 
        />}
        {loginEmailOpen && <LoginWithEmailModal key="login-email-modal" isOpen={loginEmailOpen} onClose={() => setLoginEmailOpen(false)} />}
        {signupEmailOpen && <SignupWithEmailModal key="signup-email-modal" isOpen={signupEmailOpen} onClose={() => setSignupEmailOpen(false)} onContinue={handleContinueWithEmail} />}
        {otpModalOpen && <VerifyPhoneOrEmailOtpModal key="otp-modal" isOpen={otpModalOpen} type="email" identifier={otpIdentifier} onClose={() => setOtpModalOpen(false)} onVerified={handleOtpVerified} />}
        {completeProfileOpen && <CompleteProfileModal key="complete-profile-modal" isOpen={completeProfileOpen} onClose={() => setCompleteProfileOpen(false)} onOpenLoginModal={() => setLoginEmailOpen(true)} />}
        {loginAlertOpen && <LoginAlertModal key="login-alert-modal" isOpen={loginAlertOpen} onClose={() => setLoginAlertOpen(false)} onOpenChooseModal={() => setSignupChooseModalOpen(true)} />}
      </AnimatePresence>
